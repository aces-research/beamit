...
class NewtonRaphsonSolver:
    ...
    def solve(self, Nmax = 10):
        ...
        if (Dirichlet_dofs.size == 0):
            sys.exit("\nDirichlet boundary conditions not found.")
        else:
        ...
        for i in range(0, Nmax):
            ...
            if (np.linalg.det(self.A) == 0.0):
                sys.exit("\nSystem not fixed properly.")
            # check if loads are applied on the system
            # include a check for elemental loads later!!!
            elif (np.linalg.norm(nodal_loads, ord=2) == 0.0):
                sys.exit("\nSystem not loaded.")
            elif (i == 0):
                print("\nNewton-Raphson iterations started!!!")
            ...
            # assess convergence (add elemental loads later!!!)
            # the current external forces
            f_ext_elemental = np.zeros([self.system.nequations, 1])
            f_ext = nodal_loads + f_ext_elemental
            ...
            # the relative residual norm at all the NEUMANN BOUNDARIES
            rel_L2_norm = np.linalg.norm(updated_residual[Neumann_dofs], ord=2)/np.linalg.norm(f_ext[Neumann_dofs], ord=2)
            print("\nIteration:",i+1,",","Relative L2-norm = ",rel_L2_norm)
